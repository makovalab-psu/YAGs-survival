...
GENES_COL1 = ['TSPY', 'BPY2', 'VCY']
GENES_COL2 = ['RBMY', 'HSFY']
GENES_COL3 = ['CDY', 'DAZ']
ALL_GENES  = GENES_COL1 + GENES_COL2 + GENES_COL3
...
PANEL_H       = 30
UNIT_H        =  2.5  # height per isoform in 'count' normalize mode    # height of each species panel (overridable)
...
LABEL_LEFT  =  9.0   # space for gene+species labels to the left of X1
LABEL_RIGHT =  2.0   # space for cluster labels to the right of X3+BAR_W
COL_GAP     =  1.0   # data-unit gap between the two gene columns

# x-offset per column (each column is LABEL_LEFT + X3 + BAR_W + LABEL_RIGHT + COL_GAP wide)
XOFF = LABEL_LEFT + X3 + BAR_W + LABEL_RIGHT + COL_GAP  # ≈ 37

# Full figure x-range (3 columns)
XLIM_LEFT  = -LABEL_LEFT
XLIM_RIGHT = 2 * XOFF + X3 + BAR_W + LABEL_RIGHT + 0.3
...
def compute_column_layout(all_gene_data: dict, gene_list: list,
    ...
    for gi, gene in enumerate(gene_list):
        ...
        y_cursor += 6.0  # padding above bars (gene label stays at y_start)
        gene_bars_start = y_cursor

        # Per-species totals (reads + isoform counts)
        species_reads_total: dict = {}
        species_seq_counts: dict = {}
        for sp in gdata['species']:
            seqs_sp = [seq for seqs in gdata['protein_groups'].values()
                       for seq in seqs if extract_species(seq) == sp]
            species_reads_total[sp] = sum(extract_reads(s) for s in seqs_sp)
            species_seq_counts[sp]  = len(seqs_sp)
        gene_seq_reads = sum(species_reads_total.values())

        if normalize == 'count':
            species_panel_h = {
                sp: species_seq_counts[sp] * UNIT_H
                    + max(0, species_seq_counts[sp] - 1) * INTRA_BAR_GAP
                for sp in gdata['species']
            }
        elif normalize is not False:  # True or 'hybrid' → equal panels
        ...
        for i, species in enumerate(gdata['species']):
            ...
            bar_weight = (lambda s: 1.0) if normalize == 'count' else extract_reads
            entries = []
            for rep, seqs in gdata['protein_groups'].items():
                for seq in seqs:
                    if extract_species(seq) == species:
                        iso = extract_isoform(seq, gene)
                        entries.append((seq, bar_weight(seq), iso, rep))
            ...
            for rep in seen_reps:
                ...
                h = UNIT_H if normalize == 'count' else (reads_here / total_reads) * avail_h2
        ...
        if normalize == 'count':
            # Each protein = 1 unit; species fractions sum to 1 per protein
            protein_w = {prot: 1.0 for prot in gdata['protein_total_reads']}
        else:
            protein_w = {
        ...
        for cluster_rep in sorted(gdata['cluster_groups'].keys()):
            ...
            h = cluster_w * UNIT_H if normalize == 'count' else \
                (cluster_w / total_w) * gene_total_h if total_w > 0 else 0

            # Per-protein per-species weights (fractions summing to protein_w per protein)
            prot_sp_w = {}
            for prot in prot_reps:
                sp_w: dict = defaultdict(float)
                seqs = gdata['protein_groups'].get(prot, [])
                if normalize == 'count':
                    n = len(seqs) or 1
                    for seq in seqs:
                        sp_w[extract_species(seq)] += 1.0 / n
                else:
                    for seq in seqs:
...
def print_cluster_summary(all_gene_data: dict) -> None:
    for gene in ALL_GENES:
        if gene not in all_gene_data:
            continue
        gdata = all_gene_data[gene]
        print(f'\n{gene}')
        for crep, label in sorted(gdata['cluster_labels'].items(), key=lambda x: x[1]):
            n = len(gdata['cluster_groups'].get(crep, []))
            print(f'  {label}: {crep}  ({n} members)')
...
def main():
    ...
    print_cluster_summary(all_gene_data)

    # Col1 layout is the height reference; col2 and col3 scale to match it
    col1_layout, col1_h = compute_column_layout(all_gene_data, GENES_COL1, normalize=NORMALIZE)

    def panel_h_for(genes: list, target_h: float) -> float:
        present = [g for g in genes if g in all_gene_data]
        n = sum(len(all_gene_data[g]['species']) for g in present)
        fixed = (sum(max(0, len(all_gene_data[g]['species']) - 1) for g in present) * PANEL_GAP
                 + max(0, len(present) - 1) * GENE_GAP)
        return (target_h - fixed) / n if n else PANEL_H

    col2_layout, col2_h = compute_column_layout(all_gene_data, GENES_COL2,
                                                 panel_h=panel_h_for(GENES_COL2, col1_h),
                                                 normalize=NORMALIZE)
    col3_layout, col3_h = compute_column_layout(all_gene_data, GENES_COL3,
                                                 panel_h=panel_h_for(GENES_COL3, col1_h),
                                                 normalize=NORMALIZE)
    total_h = max(col1_h, col2_h, col3_h)

    # Figure: exactly 180 mm wide; height at 0.26 mm per data unit
    MM_PER_UNIT_Y = 0.26
    y_span = total_h + 18   # ylim goes from -15 to total_h+3
    fig_w_mm = 180
    fig_h_mm = y_span * MM_PER_UNIT_Y + 30
    ...
    # Column headers (repeated for each of the three columns)
    col_headers = [(X1, 'Transcript\nisoform'), (X2, 'Protein\nisoform'), (X3, 'Protein\ncluster')]
    for col_xoff in (0.0, XOFF, 2 * XOFF):
        for x, label in col_headers:
            ax.text(col_xoff + x + BAR_W / 2, -7.5, label,
                    ha='center', va='center', fontsize=FS_LG, fontweight='bold')

    draw_gene_column(ax, col1_layout, all_gene_data, GENES_COL1, xoff=0.0)
    draw_gene_column(ax, col2_layout, all_gene_data, GENES_COL2, xoff=XOFF)
    draw_gene_column(ax, col3_layout, all_gene_data, GENES_COL3, xoff=2 * XOFF)
...
if __name__ == '__main__':
    _MODES = {'normalized': True, 'absolute': False, 'hybrid': 'hybrid', 'count': 'count'}
    parser = argparse.ArgumentParser()
    parser.add_argument('--mode', choices=_MODES, default='hybrid',
                        help='Height mode: normalized | absolute | hybrid | count  (default: hybrid)')
    args = parser.parse_args()
    NORMALIZE = _MODES[args.mode]
    main()
