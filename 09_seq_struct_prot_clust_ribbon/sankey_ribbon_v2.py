...
def compute_column_layout(all_gene_data: dict, gene_list: list,
    ...
    for gi, gene in enumerate(gene_list):
        ...
        for cluster_rep in sorted(gdata['cluster_groups'].keys()):
            ...
            cluster_w = sum(protein_w.get(p, 0) for p in prot_reps_sorted)
            h = (cluster_w / total_w) * gene_total_h if total_w > 0 else 0

            protein_slots, yr = {}, y3
            for prot in prot_reps_sorted:
                pw = protein_w.get(prot, 0)
                ph = (pw / cluster_w) * h if cluster_w > 0 else 0
                ...
                for sp in SPECIES_ORDER:
                    if sp in sp_w:
                        sph = (sp_w[sp] / pw) * ph if pw > 0 else 0
...
def draw_gene_column(ax, layout, all_gene_data, gene_list, xoff=0.0):
    ...
    for gene in gene_list:
        ...
        # Gene label: horizontal italic, right-aligned, at top of gene group
        ax.text(xoff + X1 - 0.4, gene_layout['y_start'], gene,
                ha='right', va='top', fontsize=FS_LG,
