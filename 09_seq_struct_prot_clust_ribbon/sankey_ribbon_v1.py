...
SPECIES_LABELS = {
    ...
    'PonAbe': 'Bornean orang.', 'PonPyg': 'Sumatran orang.',
}

# Two-column gene arrangement
LEFT_GENES  = ['BPY2', 'CDY', 'HSFY', 'VCY']
RIGHT_GENES = ['RBMY', 'TSPY']
...
def load_gene_data(gene: str) -> dict:
    ...
    # Cluster colours: weighted blend of isoform colours of all sequences in cluster
    cluster_colors = {}
    for crep in sorted_creps:
        ...
        for prot_rep in cluster_groups[crep]:
            for seq in protein_groups.get(prot_rep, []):
                ...
                col = ISOFORM_COLORS.get(iso, DEFAULT_COLOR)
...
def compute_column_layout(all_gene_data: dict, gene_list: list,
    ...
    for gi, gene in enumerate(gene_list):
        ...
        for i, species in enumerate(gdata['species']):
            ...
            if not entries:
                gene_layout['species_panels'][species] = {
                    'y_start': panel_y_start, 'y_end': panel_y_start + panel_h,
            ...
            n_bars = len(entries)
            avail_h = panel_h - max(0, n_bars - 1) * INTRA_BAR_GAP
            ...
            for seq, reads, iso, rep in entries:
                ...
                col1_bars.append({
                    ...
                    'color': ISOFORM_COLORS.get(iso, DEFAULT_COLOR),
                    'reads': reads, 'isoform': iso, 'protein_rep': rep,
                })
                y += h + INTRA_BAR_GAP

            # Col2: group by protein rep
            ...
            avail_h2 = panel_h - max(0, n_reps - 1) * INTRA_BAR_GAP
        ...
        gene_total_reads = sum(gdata['protein_total_reads'].values())
        ...
        for cluster_rep in sorted(gdata['cluster_groups'].keys()):
            ...
            prot_reps_sorted = sorted(
                prot_reps,
                key=lambda p: (
                    SPECIES_ORDER.index(extract_species(p))
                    if extract_species(p) in SPECIES_ORDER else 99,
                    isoform_num(extract_isoform(p, gene)),
                    -gdata['protein_total_reads'].get(p, 0),
                )
            )
            cluster_reads = sum(gdata['protein_total_reads'].get(p, 0) for p in prot_reps_sorted)
            h = (cluster_reads / gene_total_reads) * gene_total_h if gene_total_reads > 0 else 0

            protein_slots, yr = {}, y3
            for prot in prot_reps_sorted:
                pr = gdata['protein_total_reads'].get(prot, 0)
                ph = (pr / cluster_reads) * h if cluster_reads > 0 else 0
                seq_by_species = defaultdict(int)
                for seq in gdata['protein_groups'].get(prot, []):
                    seq_by_species[extract_species(seq)] += extract_reads(seq)
                species_slots, yrs = {}, yr
                for sp in SPECIES_ORDER:
                    if sp in seq_by_species:
                        sp_reads = seq_by_species[sp]
                        sph = (sp_reads / pr) * ph if pr > 0 else 0
                        species_slots[sp] = (yrs, yrs + sph)
                        yrs += sph
                protein_slots[prot] = {'y_bot': yr, 'y_top': yr + ph, 'species': species_slots}
                yr += ph
            ...
            col3_bars.append({
                ...
                'y_bot': y3, 'y_top': y3 + h,
                'reads': cluster_reads,
                'proteins': prot_reps_sorted, 'protein_slots': protein_slots,
            })
            y3 += h
...
def bezier_ribbon(ax, x0, y0_bot, y0_top, x1, y1_bot, y1_top, color, alpha=0.35):
    if y0_top - y0_bot < 0.05 or y1_top - y1_bot < 0.05:
        return
    cx = (x0 + x1) / 2
    verts = [
...
def draw_gene_column(ax, layout, all_gene_data, gene_list, xoff=0.0):
    ...
    for gene in gene_list:
        ...
        # Gene label (rotated bold, left of col1)
        gene_mid = (gene_layout['y_start'] + gene_layout['y_end']) / 2
        ax.text(xoff + X1 - LABEL_LEFT + 1.5, gene_mid, gene,
                ha='center', va='center', fontsize=FS_LG,
        ...
        # Col3 bars + labels (spread overlapping labels with leader lines)
        ...
        natural_ys = [(bar3['y_bot'] + bar3['y_top']) / 2 for bar3 in col3_bars]
        spread_ys  = spread_labels_1d(natural_ys, FS_MD * 0.35 / 0.26)  # ~min 1 line height in data units
        label_x = xoff + X3 + BAR_W + 0.3
        for bar3, nat_y, spr_y in zip(col3_bars, natural_ys, spread_ys):
        ...
        for species, panel in species_panels.items():
            ...
            for bar1 in col1_bars:
                ...
                if col2_b:
                    ...
                    if slot:
                        bezier_ribbon(ax,
                            xoff + X1 + BAR_W, bar1['y_bot'], bar1['y_top'],
                            xoff + X2, slot[0], slot[1],
                            bar1['color'])

            # Ribbons col2 → col3 (use cluster colour)
            for bar2 in col2_bars:
                ...
                if not sp_slot:
                    continue
                bezier_ribbon(ax,
                    xoff + X2 + BAR_W, bar2['y_bot'], bar2['y_top'],
                    xoff + X3, sp_slot[0], sp_slot[1],
                    bar2['color'])   # isoform colour flows into cluster
...
def main():
    ...
    left_layout, left_h = compute_column_layout(all_gene_data, LEFT_GENES)
    ...
    right_layout, right_h = compute_column_layout(all_gene_data, RIGHT_GENES, panel_h=panel_h_right)
    total_h = max(left_h, right_h)

    # Figure: exactly 180 mm wide; height at 0.42 mm per data unit
    MM_PER_UNIT_Y = 0.26
    y_span = total_h + 13   # ylim goes from -10 to total_h+3
    fig_w_mm = 180
    ...
    ax.set_ylim(total_h + 3, -10)
    ax.axis('off')
    fig.patch.set_facecolor('white')

    # Column headers for left gene column
    for x, label in [(X1, 'Seq+struct isoform'), (X2, 'Protein'), (X3, 'Cluster')]:
        ax.text(x + BAR_W / 2, -7, label,
                ha='center', va='bottom', fontsize=FS_LG, fontweight='bold')

    # Column headers for right gene column
    for x, label in [(X1, 'Seq+struct isoform'), (X2, 'Protein'), (X3, 'Cluster')]:
        ax.text(XOFF + x + BAR_W / 2, -7, label,
                ha='center', va='bottom', fontsize=FS_LG, fontweight='bold')
